        ...
        for oiter in range(optim_iter):
            ...
            for kk in range(prerollout+one_period):
                ...
                if kk >= prerollout:
                    ...
                    if oiter == optim_iter - 1:
                        try:    
                            os.makedirs("./optimized_traj_fno_BP_rebuttal_na/test_{:06d}/sim_{:06d}".format(testnum, datanum))
                        except Exception:
                            pass   
                        with open('./optimized_traj_fno_BP_rebuttal_na/test_{:06d}/sim_{:06d}/feature_{:06d}.npy'.format(testnum, datanum, kk), 'wb') as f:
