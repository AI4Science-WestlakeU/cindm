...
for testnum in range(10):
    ...
    #                 # cen = cen.to(device)       
                if kk >= prerollout:
                    ...
                    if oiter == optim_iter - 1:
                        try:    
                            os.makedirs("./optimized_traj_lepde_CEM_twobds_rebuttal/test_{:06d}/sim_{:06d}".format(testnum, datanum))
                        except Exception:
                            pass   
                        with open('./optimized_traj_lepde_CEM_twobds_rebuttal/test_{:06d}/sim_{:06d}/feature_{:06d}.npy'.format(testnum, datanum, kk), 'wb') as f:
