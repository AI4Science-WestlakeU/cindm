...
def metric_batch(forces_array_batch, lam=1, avg_over_time_first=True):
    # forces_array_batch: (batch_size, 100, num_boundaries, 3) # 100 is the time steps, 3 is x, y, z forces
    forces_array_batch = forces_array_batch[:,:,:,0:2] # ignore zero z force, output forces_array_batch size: (batch_size, 100, num_boundaries, 2)
    drag = np.sum(forces_array_batch[:,:,:,0], axis=2) # drag force sum over boundaries, output drag size: (batch_size, 100)
    lift = np.sum(forces_array_batch[:,:,:,1], axis=2) # lift force sum over boundaries, output lift size: (batch_size, 100)
    drag_mean = np.mean(drag, axis=1) # average over time steps, output drag_mean size: (batch_size)
    lift_mean = np.mean(lift, axis=1) # average over time steps, output lift_mean size: (batch_size)
    drag_min = np.min(abs(drag_mean))
    lift_max = np.max(abs(lift_mean))  

    if avg_over_time_first:
        obj_mean = metric(lift_mean, drag_mean, lam, use_frac=False) 
        lift_over_drag_mean = metric(lift_mean, drag_mean, lam, use_frac=True) 
    else:
        obj = metric(lift, drag, lam, use_frac=False) 
        lift_over_drag = metric(lift, drag, lam, use_frac=True) 
        obj_mean = np.mean(obj, axis=1) # average over time steps, output obj_mean size: (batch_size)
...
if __name__ == "__main__":
    # root_dir = "double"
    # root_dir = "./optimized_traj_fno_BP/" # the folder of two cell in Table 3
    # root_dir = "./optimized_traj_lepde_BP/" # the folder of two cell in Table 3
    # root_dir = "./optimized_traj_fno_CEM/" # the folder of two cell in Table 3
    # root_dir = "./optimized_traj_fno_BP_twobds/" # the folder of two cell in Table 3
    # root_dir = "./optimized_traj_fno_CEM_twobds/"
    # root_dir = "./optimized_traj_lepde_BP_twobds/" # the folder of two cell in Table 3
    # root_dir = "./optimized_traj_lepde_CEM/" # the folder of two cell in Table 3
    # root_dir = "./optimized_traj_lepde_CEM_twobds/"
    # root_dir = "./optimized_traj_fno_BP_threebds/"
    # root_dir = "./optimized_traj_fno_CEM_threebds/"
    # root_dir = "./optimized_traj_lepde_BP_threebds/"
    root_dir = "./optimized_traj_lepde_CEM_threebds/"
