...
for simnum in range(1000):
    if is_visualize:
        pdf = PdfPages('../visualization/naca_ellipse_boundary_sim_{:06d}.pdf'.format(simnum))
    for i in range(1):
        f = open("../boundary/sim_{0}/boundary_{1}.txt".format(simnum, i))
        data = f.read()
        ...
        np.save("../np_boundary/boundary_{:06d}".format(simnum), np.stack([npb[0::2], npb[1::2]]))
