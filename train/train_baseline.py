...
if args.is_pretrain_autoencode:
    ...
    for epoch_pretrain in range(args.epochs_pretrain):
        ...
        for j_pretrain, data in enumerate(train_loader):
            p.print(f"taining on {j}th data ")
            data.to(device)
...
while epoch < args.epochs:
    ...
    import pdb
    # pdb.set_trace()
    p.print("0", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
    for j, data in enumerate(train_loader):
        p.print("0.00", precision="millisecond", is_silent=args.is_timing < 2, avg_window=1)
        # p.print(f"train {j}th data in epoch {epoch}")
        # pdb.set_trace()
        t_end = time.time()
        ...
        if args.algo.startswith("supn") and j % 20 == 0:
            ...
            kwargs["isplot"] = False
        # if args.is_offmask:
        #     kwargs["is_offmask"] = True
        # else:
        #     kwargs["is_offmask"] = False
        opt.zero_grad()

        args_core = update_args(args, "multi_step", "1") if epoch < args.multi_step_start_epoch else args
        p.print("1.00", precision="millisecond", is_silent=args.is_timing < 2, avg_window=1)
        if args.dp_mode == "dp" :
            loss = get_loss(
                model,
                data,
        ...
        p.print("2.00", precision="millisecond", is_silent=args.is_timing < 2, avg_window=1)
        ...
        p.print("3.00", precision="millisecond", is_silent=args.is_timing < 2, avg_window=1)  
    for key, item in train_info.items():
        train_info[key] = np.mean(item)
    train_loss = total_loss / count
    p.print("5.00", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
    if epoch % args.test_interval == 0 or epoch == args.epochs - 1:
        ...
        #     **kwargs
        # )
        p.print("5.01", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
        test_loss, test_info = test(
            test_loader, model, device, args,
            density_coef=0, current_epoch=epoch, current_minibatch=0,
            **kwargs
        )
        p.print("6.00", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
        val_loss=None
        if val_loss is None:
            val_loss = test_loss
            val_info = test_info
    ...
    p.print("7.00", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
    ...
    if args.lr_scheduler_type == "rop":
        ...
        scheduler.step()
    p.print("7.10", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
    ...
    if is_diagnose(loc="3", filename=filename):
        pdb.set_trace()
    p.print("8.00", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
    if epoch % args.save_interval == 0 and epoch >= 0:
        print("aha")
        record_data(data_record, [epoch, get_model_dict(model)], ["save_epoch", "model_dict"])
        p.print("8.10", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
        # with open(filename, "wb") as f:
        #     pickle.dump(data_record, f)
    p.print("9.00", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
    ...
    if True:
        ...
        p.print("9.10", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
        with open(filename, "wb") as f:
            pickle.dump(data_record, f)
        p.print("9.20", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
    if "to_stop" in locals() and to_stop:
        p.print("Early-stop at epoch {}.".format(epoch))
        break
    epoch += 1
    p.print("9.30", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
record_data(data_record, [epoch, get_model_dict(model)], ["save_epoch", "model_dict"])
with open(filename, "wb") as f:
    pickle.dump(data_record, f)
    p.print("10.00", precision="millisecond", is_silent=args.is_timing < 1, avg_window=1)
