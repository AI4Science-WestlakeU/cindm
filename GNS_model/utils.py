...
def compute_binary_pressForce(pressure, np_clustered_grid, is_boundary_plot=False):
    ...
    for i in range(len(final_boundaries)):
        ...
        # Compute force
        pressures[i+1] = compute_pressForce(pressure, torch.tensor(boundary.copy())+0.5)
    return pressures
...
class CustomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        #     if Flag==1:
        #         indices.append(i)
        # indices_np=np.load("/user/project/inverse_design/dataset/nbody_dataset/nbody-2/speed-300/customerSampler_indices.npy")
